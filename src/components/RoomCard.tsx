...
export default function RoomCard({
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -3 }}
      whileTap={{ scale: 0.98 }}
      className={`rounded-3xl p-5 sm:p-6 border cursor-pointer backdrop-blur-xl transition-all duration-300 relative overflow-hidden group glass-noise flex flex-col justify-between ${
        isSelected 
          ? darkMode
    ...
    >
      ...
      {/* Bottom Action Area: Activity Status + Interactive Arrow Action Button */}
      <div className={`pt-3 border-t flex justify-between items-center text-[10px] relative z-10 ${
        ...
        <span className="text-[11px] font-bold text-slate-400 truncate pr-2">
          {activeDeviceCount > 0 ? (
            ...
            <span>All devices idle</span>
          )}
        </span>
        
        <button
          type="button"
          id={`btn-focus-room-${room.id}`}
          onClick={(e) => {
            e.stopPropagation();
            onSelect();
          }}
          title={`Focus on ${room.name}`}
          className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all cursor-pointer shadow-xs ${
            isSelected 
              ? 'bg-[#7B61FF] text-white shadow-md shadow-[#7B61FF]/40 scale-105' 
              : darkMode 
