...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-2xl shadow-xl shadow-slate-200/80 dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] bg-white/95 dark:bg-slate-900/70 text-slate-900 dark:text-white backdrop-blur-xl", className)}>
                {children}
            </div>
...
const TableCardHeader = ({ title, badge, description, contentTrailing, className }: TableCardHeaderProps) => {
    ...
    return (
        <div
            className={cx(
                "relative flex flex-col items-start gap-4 border-b border-slate-200/80 dark:border-white/10 px-4 md:flex-row",
        ...
        >
            <div className="flex flex-1 flex-col gap-0.5">
                <div className="flex items-center gap-2">
                    <h2 className="text-md font-bold tracking-tight text-slate-900 dark:text-white">{title}</h2>
                    {badge ? (
                ...
                {description && <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{description}</p>}
            </div>
            {contentTrailing}
        </div>
    );
        ...
        <AriaTableHeader
            ...
            className={(state) =>
                cx(
                    ...
                    bordered &&
                        "[&>tr>th]:after:pointer-events-none [&>tr>th]:after:absolute [&>tr>th]:after:inset-x-0 [&>tr>th]:after:bottom-0 [&>tr>th]:after:h-px [&>tr>th]:after:bg-slate-200/80 dark:[&>tr>th]:after:bg-white/10 [&>tr>th]:focus-visible:after:bg-transparent",
            ...
            {(state) => (
                <AriaGroup className="flex items-center gap-1">
                    <div className="flex items-center gap-1">
                        {label && <span className="text-xs font-semibold whitespace-nowrap text-slate-500 dark:text-slate-400">{label}</span>}
...
const TableRow = <T extends object>({ columns, children, className, highlightSelectedRow = true, size: sizeProp, ...props }: TableRowProps<T>) => {
    ...
    return (
        <AriaRow
            ...
            className={(state) =>
                cx(
                    "relative outline-focus-ring transition-colors after:pointer-events-none hover:bg-black/5 dark:hover:bg-white/5 focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" ? "h-14" : "h-18",
                    highlightSelectedRow && "selected:bg-black/5 dark:selected:bg-white/5",

                    // Row border—using an "after" pseudo-element to avoid the border taking up space.
                    "[&>td]:after:absolute [&>td]:after:inset-x-0 [&>td]:after:bottom-0 [&>td]:after:h-px [&>td]:after:w-full [&>td]:after:bg-slate-100 dark:[&>td]:after:bg-white/5 last:[&>td]:after:hidden [&>td]:focus-visible:after:opacity-0 focus-visible:[&>td]:after:opacity-0",
