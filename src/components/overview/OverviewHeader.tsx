...
export default function OverviewHeader({ darkMode = true }: OverviewHeaderProps) {
  ...
  // Primary User Info for edge-to-edge picture
  const primaryUser = homeUsers[0] || userEntities[0];
  const primaryUserPicture = getHAImageUrl(primaryUser?.attributes?.entity_picture, serverUrl);
    ...
    <section className="space-y-4 mb-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {userEntities.map((user) => {
          ...
          return (
            <button
              ...
              className={`h-9 pl-0 pr-3 rounded-full text-xs font-bold transition-all cursor-pointer hover:scale-105 active:scale-95 border flex items-center gap-2 shadow-xs overflow-hidden ${
                isHome
                  ? darkMode
        ...
        >
          ...
          <span>{isPlayingMedia ? 'Playing Audio' : 'Audio'}</span>
        </button>
      ...
      {/* ------------------------------------------------------------- */}
      {/* 2. BENTO TILES GRID (MATCHING UNIFORM HEIGHT h-36)            */}
      {/* ------------------------------------------------------------- */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3.5">
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{onLights.length}</span>
        ...
        >
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border shadow-xs transition-all ${
              ...
              <ToggleRight size={22} weight="duotone" />
            </div>
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{onSwitches.length}</span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {switchEntities.length} Active</span>
            </div>
          </div>

          <div className="relative z-10">
            <div className="text-sm font-bold text-slate-900 dark:text-white">Switches</div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate flex items-center justify-between">
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{activeFans.length}</span>
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className={`text-2xl font-black font-mono ${openDoors.length > 0 ? 'text-amber-600 dark:text-amber-300' : 'text-slate-900 dark:text-white'}`}>
                {openDoors.length}
              </span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {doorSensors.length} Doors</span>
            </div>
          </div>
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className={`text-2xl font-black font-mono ${openWindows.length > 0 ? 'text-amber-600 dark:text-amber-300' : 'text-slate-900 dark:text-white'}`}>
                {openWindows.length}
              </span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {windowSensors.length} Windows</span>
            </div>
          </div>
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{activeMotion.length}</span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {motionSensors.length} Active</span>
            </div>
          </div>

          <div className="relative z-10">
            <div className="text-sm font-bold text-slate-900 dark:text-white">Motion Zones</div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate flex items-center justify-between">
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className={`text-2xl font-black font-mono ${activeLeaks.length > 0 ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`}>
                {activeLeaks.length}
              </span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {leakSensors.length} Probes</span>
            </div>
          </div>

          <div className="relative z-10">
            <div className="text-sm font-bold text-slate-900 dark:text-white">Water Leaks</div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate flex items-center justify-between">
        ...
        >
          ...
          <div className="relative z-10 my-0.5">
            <div className="flex items-baseline gap-1.5">
              <span className={`text-2xl font-black font-mono ${activeSmoke.length > 0 ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`}>
                {activeSmoke.length}
              </span>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">/ {smokeSensors.length} Detectors</span>
            </div>
          </div>
